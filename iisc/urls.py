...
urlpatterns = patterns('',
    ...
    url(r'^fn/(?P<fn>\w+)/rate/(?P<user_id>\d+)/(?P<rating>\d+)/$', views.rate,{'string': True}, name='rate_str'),
    url(r'^register/$', views.register, name='register'),
    url(r'^fn/(?P<fn>\d+)/save/(?P<user_id>\d+)/$', views.save_fid, name='save'),
    url(r'^fn/(?P<fn>\w+)/save/(?P<user_id>\d+)/$', views.save_fid,{'string': True}, name='save_str'),
)
